    ...
    for sensor_description in BINARY_SENSORS:
        if sensor_description.key.startswith("inverters_"):
            ...
            if coordinator.data.get("relay_info") != None:
                for serial_number in coordinator.data["relay_info"]:
...
class EnvoyRelayEntity(EnvoyBinaryEntity):
    ...
    @property
    def relay(self):
        return self.coordinator.data.get("relays", {}).get(self._device_serial_number)
    ...
    @property
    def relay_info(self):
        return self.coordinator.data.get("relay_info", {}).get(
            self._device_serial_number
        )
    ...
    def available(self) -> bool:
        ...
        if self.value_key != "communicating":
            return self.relay_info.get("communicating")
...
class EnvoyRelayGenericEntity(EnvoyRelayEntity):
    ...
    def is_on(self) -> bool | None:
        ...
        return self.relay_info.get(self.value_key)
